...
namespace Live2D.Cubism.Framework.Expression
{
    ...
    public class CubismExpressionController : MonoBehaviour, ICubismUpdatable
    {
        ...
        private readonly List<CubismExpressionData> _resetRequestMotions = new();
        ...
        #endregion
        public void RequestResetExpression(CubismExpressionData expressionData)
        {
            if (expressionData != null)
            {
                _resetRequestMotions.Add(expressionData);
            }
        }
        /// <summary>
        ...
        public void OnLateUpdate()
        {
            ...
            if (_resetRequestMotions.Count > 0)
            {
                foreach (var motion in _resetRequestMotions)
                {
                    foreach (var paramInfo in motion.Parameters)
                    {
                        var param = _model.Parameters.FindById(paramInfo.Id);
                        if (param != null)
                        {
                ...
                _resetRequestMotions.Clear(); // 清空请求
            }
        }
